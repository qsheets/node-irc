...
Client.prototype.join = function(channel, key, callback) { // {{{
    ...
    channel.split(',').forEach(function(chan) {
        self.once('join' + chan, function () {
            ...
            if (self.opt.channels.indexOf(chan) == -1) {
                self.opt.channels.push(chan);
            }
...
Client.prototype.part = function(channel, message, callback) { // {{{
    ...
    channel.split(',').forEach(function(chan) {
        if ( typeof(callback) == 'function' ) {
            self.once('part' + channel, callback);
        }
    
        // remove this channel from this.opt.channels so we won't rejoin
        // upon reconnect
        if (self.opt.channels.indexOf(chan) != -1) {
            self.opt.channels.splice(self.opt.channels.indexOf(chan), 1);
        }
    });
...
Client.prototype.msg = function(target, text) { // {{{
    var self = this;
...
Client.prototype.ctcp = function(to, type, text) { // {{{
    return this[type === 'request' ? 'msg' : 'notice'](to, '\1'+text+'\1');
} // }}}
